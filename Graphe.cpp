...
Graphe::Graphe()
{
    cout << "Nombre de noeuds du graphe ?"<<endl;
    cin >> n;
    g = new int * [n];
    int i,j;
    for (i=0;i<n;i++)
    ...
    cout << "Saisie la valeur 0 pour la distance d'un à meme noeud" <<endl;

    for (i=0;i<n;i++)
        for(j=0;j<n;j++)
        {
        cout  <<"distance arc "<<"("<<i<<","<<")"<<endl;
        cin >>g[i][j];
        }
        distmin = new int[n];
...
void Graphe::afficher()
{
    ...
    for (i=0;i<n;i++)
    {
        ...
        if (distmin[i]!=9999)
            cout<<"distance minimale:"<<distmin;
        else
            cout<<"pas se chemin !!!!!"<<distmin;
    }
}
